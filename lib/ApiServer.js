...
class ApiServer {
   ...
   * @private
   */
  _executeMiddleware(middlewareName, req, route) {
   ...
   * @private
   */
  _executeController(controllerName, method, req, route) {
  ...
  _registerRoutes(routes) {
    routes.forEach((route) => {
      this._server[route.method](route.path, (req, res) => {
        const requestLogger = this._config.get('app.server.logger');
        if (!TypeUtils.isUndefined(requestLogger)) {
          this._executeMiddleware(ApiServer.coreMiddlewares.logger, req, route);
        }
        ...
        let response;
        if (route.validate) {
          response = this._executeMiddleware(
        ...
        if (TypeUtils.isUndefined(response)) {
          ...
          response = this._executeController(action[0], action[1], req, route);
        }
        ...
        if (response.isType(ApiResponse.TYPES.JSON)) {
